...
// if everything is ok, try to upload file
    ...
    if ($result = $conn->query("SELECT * FROM brand")) 
    {
        ...
        $sql = "INSERT INTO brand (id, image_name, description)
        ...
        if (mysqli_query($conn, $sql)) {
            for($i = 0; $i < 1; $i++) 
            {
                ...
                if ($i == 0)
                {
                    ...
                    if (move_uploaded_file($_FILES["fileToUpload1"]["tmp_name"],"upload_brand/" . $newfilename1)) 
                    {
